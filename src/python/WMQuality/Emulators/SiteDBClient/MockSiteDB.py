...
class MockSiteDB(object):
    """
    API for dealing with retrieving information from SiteDB
    """
    def __init__(self, config={}):
        print("Initializing SiteDB object")
        self.sitedb = MockSiteDBApi()
    ...
    def getJSON(self, callname, filename = 'result.json', clearCache = False, verb = 'GET', data={}):
        ...
        result = ''
        result = self.sitedb.genericLookup(callname, filename = 'result.json', clearCache = False, verb = 'GET', data={})
    ...
    def _sitenames(self, sitename=None, clearCache=False):
        ...
        sitenames = self.getJSON(callname='site-names', filename=filename, clearCache=clearCache)
        if sitename:
